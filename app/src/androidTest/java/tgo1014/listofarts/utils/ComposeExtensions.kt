...
fun ComposeContentTestRule.assertContentDescriptionExists(
    text: String,
    substring: Boolean = false,
    ignoreCase: Boolean = false,
    useUnmergedTree: Boolean = false
): SemanticsNodeInteraction {
    return this.onNodeWithContentDescription(text, substring, ignoreCase, useUnmergedTree)
        .assertExists()
}
...
fun ComposeContentTestRule.assertContentDescriptionDoesNotExists(
    text: String,
    substring: Boolean = false,
    ignoreCase: Boolean = false,
    useUnmergedTree: Boolean = false
) {
    this.onNodeWithContentDescription(text, substring, ignoreCase, useUnmergedTree)
        .assertDoesNotExist()
}
